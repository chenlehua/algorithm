...
**核心思想**：通过**串行**训练多棵决策树，每棵树学习前面所有树的**残差（预测误差）**，最终将所有树的预测结果相加。
...
其中每棵树 $f_m$ 拟合的目标是**负梯度（残差的近似）**：
$$f_m(x) \approx -\frac{\partial L(y, F_{m-1}(x))}{\partial F_{m-1}(x)}$$
...
└─────────────────────────────────────────┘
              ...
              重复M次...
                    ↓
最终预测：F_M(x) = f₁(x) + f₂(x) + ... + f_M(x)
```

**直观理解**：
- 第1棵树：粗略预测，误差较大
- 第2棵树：专门学习第1棵树"哪里预测错了"
- 第3棵树：继续修正前2棵树的残余误差
- ...
- 逐步逼近真实值，类似"**查漏补缺**"
...
```python
# 输入：训练数据 D = {(xᵢ, yᵢ)}, 树的数量 M, 学习率 η

# 1. 初始化：用常数初始化预测值
F₀(x) = argmin_c Σ L(yᵢ, c)  # 通常是 y 的均值
