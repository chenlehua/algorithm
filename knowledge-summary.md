...
**问题**：直接学习"每对特征的交叉权重"参数太多

```
假设有1000个特征：
  直接方法：学习 1000×1000 = 100万个交叉权重 😱
  大部分组合没有训练数据，学不到
```

**FM的聪明做法**：给每个特征一个"小向量"，交叉强度=向量内积
...
计算交叉强度（向量内积）：
  青年×电竞 = 0.8×0.9 + 0.2×0.1 + 0.1×0.2 = 0.76 ← 强关联！
  青年×母婴 = 0.8×0.2 + 0.2×0.8 + 0.1×0.3 = 0.35 ← 弱关联
  青年×晚上 = 0.8×0.6 + 0.2×0.3 + 0.1×0.4 = 0.58 ← 中等关联
```
...
```
训练数据中：
  ✓ "王者荣耀 + 游戏手柄" 出现过 → 学到两者向量相近
  ✗ "原神 + 游戏手柄" 没出现过

但是：
  原神的向量 ≈ 王者荣耀的向量（都是游戏）
  → 计算内积时，原神×游戏手柄 也会得到高分
  → 泛化成功！
```

**一句话**：FM用"向量内积"代替"直接学权重"，参数少、能泛化
