...
层面1：RNN自身的缺陷
  早期Encoder使用RNN/LSTM，处理长序列时会"遗忘"：
  
  输入：[词1] → [词2] → [词3] → ... → [词100]
                                           ↓
                                      最终隐藏状态h
  
  词1的信息要经过99次传递才能到达最终状态
    → 每次传递都有信息损耗
    → 到最后，词1的信息几乎消失了
  
  即使用LSTM（门控机制），长序列仍会有信息衰减

层面2：架构设计的瓶颈
  用一个固定维度的向量承载所有信息：
  
  10个词的句子   ──→  [512维向量]
  100个词的句子  ──→  [512维向量]  ← 同样大小！
  1000个词的句子 ──→  [512维向量]
  
  无论输入多长，都压缩到同一个"瓶口"
  这就是"信息瓶颈"(Information Bottleneck)
...
传统方式：
  Encoder: [词1][词2][词3]...[词N] → 一个摘要向量 → Decoder
                                      ↑
                                    瓶颈在这里

注意力方式：
  Encoder: [词1][词2][词3]...[词N] → 保留所有位置的表示
                ↓    ↓    ↓           ↓
             [h1] [h2] [h3] ... [hN]  ← 全部保留！
                ↘    ↓    ↙
           Decoder随时按需查询任意位置
