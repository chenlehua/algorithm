...
| 策略 | 说明 | 使用场景 |
|-----|------|---------|
| **广度优先** | 先把第1层所有节点分完，再分第2层 | 传统决策树 |
| **深度优先** | 先把一个分支分到底，再处理其他 | 某些实现 |
| **最佳优先** | 每次选"收益最大"的节点分裂 | XGBoost、LightGBM |

**停止条件**：
├─ 达到最大深度（如 max_depth=3）
├─ 节点样本数太少（如 min_samples_split=3）
├─ 叶子样本数太少（如 min_samples_leaf=1）
├─ 分裂后方差减少太小（不值得分了）
└─ 满足任一条件即停止，该节点成为叶子
